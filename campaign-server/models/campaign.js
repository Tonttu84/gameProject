...
const campaignSchema = new mongoose.Schema({
      ...
      new mongoose.Schema(
        {
          ...
          // Which CHARTER_CATALOG row this company came from (R1,
          // docs/CAMPAIGN_PLAN.md "CHARTER RECRUITMENT", R-2): the id only,
          // never the resolved row — the same rule as `archetype` and
          // `upgrades`, so a rewritten blurb or a retuned composition reaches
          // campaigns in flight. Set at creation for the opening three and by
          // enrolCharter for every company a fate hands over.
          //
          // It is read for exactly ONE thing: excluding rows already on the
          // rolls from the next draft (services/charters.js eligibleCharters).
          // Nothing about the squad is looked up through it — the composition,
          // prestige and archetype were COPIED onto the document when the
          // company took service, because from that moment they are the
          // company's own and diverge (it reinforces, it takes upgrades, it
          // gets wiped). Absent on a squad from before v45 is a normal state:
          // an unknown row simply fences nothing off.
  ...
  // Standing constructions (Construction slice C2, docs/CAMPAIGN_PLAN.md C-3):
  // the CONSTRUCTION_CATALOG ids the campaign has built, in build order. The
  // list is the record of WHAT stands; the effects live where the row pointed
  // them — a campaign-side row pushed its permanent forage modifier at build
  // time (through applyEffect, like any fate), a battlefield-side row's walls
  // are DERIVED from this list at read time (walledSides), exactly as the
  // fort's own are derived from fortificationLevel above. A row appears at
  // most once: a construction stands, it does not stack.
  constructions: { type: [String], default: [] },
        ...
        new mongoose.Schema(
          {
            ...
            // The COMPANY picked alongside it, when that branch was a charter
            // (R1). Same reason as chosenSquadId above and the same shape: a
            // deferred fate comes to pass at end-day, so the pick has to ride
            // on the slot until then or the fate quietly enrols nobody. A
            // CHARTER_CATALOG id, not a squad id — the squad does not exist
            // yet. Null on every other branch.
            chosenCharterId: { type: String, default: null },
          },
          { _id: false },
        ),
      ],
      ...
      new mongoose.Schema(
        {
          ...
          // The companies this CHARTER fate offered (R1, R-6), sealed at the
          // same moment and for the same reason as missionOffer above: the
          // draw is random, so a view that redrew it would hand the player a
          // reroll every time they refreshed the screen. CHARTER_CATALOG ids;
          // the answer route accepts a pick only if it is one of these.
          //
          // No `locked` twin: a mission's near-miss names a charter the player
          // already owns and teaches why it cannot go, whereas a company that
          // was not drawn is simply one the drums did not reach. There is
          // nothing to teach and nobody to name.
          //
          // Absent on every non-charter fate, which is most of them.
          charterOffer: {
            type: new mongoose.Schema(
              {
      ...
      new mongoose.Schema(
        {
          ...
          // The SHORTLIST this caster improvises from once the script above is
          // spent (the casting AI, A-7): an UNORDERED set of engine spell ids,
          // capped at MAX_SHORTLIST_SPELLS.
          //
          // The script's sibling and its opposite. A script is an opening
          // SEQUENCE — each line once, in order (A-6) — so position is
          // priority there; here nothing is ordered, because what the engine
          // does with this list is draw from it by weighted lottery over the
          // scorer's numbers (A-2/A-7). A player who cares which one comes
          // first has said so already, in the three slots above.
          //
          // AN EMPTY LIST IS NOT A MUTE CASTER, and this is the field's one
          // rule worth remembering: empty means the WHOLE castable roster,
          // minus the battlefield globals a script alone may fire (E-3). So a
          // fresh hire behaves exactly as every caster did before this slice,
          // and narrowing the fence is a thing the player does TO a caster
          // rather than a thing he must do for him.
          //
          // Nothing prunes it, and nothing needs to, for `script`'s own reason:
          // paths are fixed at hire (M-5) and school levels only ever rise
          // (S2-7), so a line that was castable when it was chosen stays so.
