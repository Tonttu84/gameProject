...
// A campaign with materials to spend and a next-level cost, so the fortify
// button is enabled by default.
const withMaterials = (over = {}) => ({
  ...campaignFixture,
  resources: { ...campaignFixture.resources, materials: 200 },
  fortification: { level: 0, atCap: false, nextCost: 50, sides: [], ...over },
})
...
describe('camp panel — fortifications', () => {
  it('shows the next-level cost and posts the fortify action', async () => {
    ...
    spendCampaign.mockResolvedValue(
      withMaterials({ level: 1, nextCost: 100 }),
    )
    render(<App />)
    await screen.findByText(/War Council/)

    const btn = screen.getByTestId('fortify-button')
    expect(btn).toHaveTextContent('Raise to level 1 (50 materials)')
    ...
    expect(screen.getByTestId('fortify-button')).toHaveTextContent('level 2 (100 materials)')
  })
...
describe('camp panel — militia', () => {
  it('posts the militia purchase with the chosen count', async () => {
    ...
    expect(btn).toHaveTextContent('Raise 5 militia (10 food, 5 materials)')
    fireEvent.click(btn)
    await waitFor(() =>
